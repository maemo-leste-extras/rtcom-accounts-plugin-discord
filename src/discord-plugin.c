...
typedef struct _DiscordPluginClass DiscordPluginClass;
typedef struct _DiscordPlugin DiscordPlugin;

struct _DiscordPlugin
{
  RtcomAccountPlugin parent_instance;
};

struct _DiscordPluginClass
{
  RtcomAccountPluginClass parent_class;
};

ACCOUNT_DEFINE_PLUGIN(DiscordPlugin, discord_plugin,
                      RTCOM_TYPE_ACCOUNT_PLUGIN);

static void
discord_plugin_init(DiscordPlugin *self)
{
  RtcomAccountService *service;

  RTCOM_ACCOUNT_PLUGIN(self)->name = "discord";
...
create_advanced_settings_page(RtcomDialogContext *context)
{
  ...
  if (!dialog)
  {
    ...
    gchar title[200];
    const gchar *msg;
    GladeXML *xml = glade_xml_new(PLUGIN_XML_DIR "/discord-advanced.glade",
...
static void
discord_plugin_on_advanced_cb(gpointer data)
{
  GtkWidget *dialog;
...
static void
discord_plugin_context_init(RtcomAccountPlugin *plugin,
                             RtcomDialogContext *context)
{
  gboolean editing;
  AccountItem *account;
  ...
  if (editing)
  {
    ...
    rtcom_edit_connect_on_advanced(
          RTCOM_EDIT(page), G_CALLBACK(discord_plugin_on_advanced_cb),
          context);
  }
  else
  {
    gchar *username_label = g_strconcat(_("accounts_fi_discord_user_and_host"),
                                        NULL);
    ...
    rtcom_login_connect_on_advanced(
          RTCOM_LOGIN(page), G_CALLBACK(discord_plugin_on_advanced_cb),
          context);
  }
...
static void
discord_plugin_class_init(DiscordPluginClass *klass)
{
  RTCOM_ACCOUNT_PLUGIN_CLASS(klass)->context_init = discord_plugin_context_init;
}
